...
class RotatedAnchorHead(BaseDenseHead):
    ...
    def _get_targets_single(self,
        ...
        """
        # print("flat_anchors:",flat_anchors.size())
        ...
        if self.assign_by_circumhbbox is not None:
            ...
            assign_result = self.assigner.assign(
                ...
                None if self.sampling else gt_labels)
        # print("assign_result.gt_inds: ",assign_result.gt_inds.size())
        sampling_result = self.sampler.sample(assign_result, anchors,
                                              gt_bboxes) # 没有采样，直接返回的正负样本索引
    ...
    def get_targets(self,
        ...
        # print("concat_anchor_list[0]: ",concat_anchor_list[0].size())
        # print("concat_valid_flag_list[0]: ",concat_valid_flag_list[0].size())
        # compute targets for each image
        if gt_bboxes_ignore_list is None:
            gt_bboxes_ignore_list = [None for _ in range(num_imgs)]
        if gt_labels_list is None:
            gt_labels_list = [None for _ in range(num_imgs)]
        # print("concat_anchor_list: ",concat_anchor_list.size())
        results = multi_apply(
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        anchors = anchors.reshape(-1, 5)
        pos_targets = bbox_targets[bbox_weights==1]
        sigmoid_scores = torch.sigmoid(cls_score)
        max_scores, max_indices = torch.max(sigmoid_scores, dim=1)
        device = max_scores.device
        final_indices = torch.where(max_scores > 0.3, max_indices, torch.tensor(15, device=device))
        bool_tensor = (final_indices == labels)
        pos_bool = bool_tensor[(bbox_weights==1)[:,0]]
        pos_bool = pos_bool[pos_bool==True]
        neg_bool = bool_tensor[(bbox_weights!=1)[:,0]]
        neg_bool = neg_bool[neg_bool==True]
        # print("bbox_targets: ",bbox_targets[bbox_weights==1])
        # print("anchors: ",anchors[bbox_weights==1])
        # print("bbox_weights: ",bbox_weights.size())
        # print("delta: ", (bbox_targets-anchors)[bbox_weights==1])
    ...
    def loss(self,
        ...
        # print("cls_scores: ", cls_scores[4].size())
        # print("bbox_preds: ", bbox_preds[0].size())
