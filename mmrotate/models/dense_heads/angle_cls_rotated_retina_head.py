...
class AngleCLSRRetinaHead(RotatedRetinaHead):
    ...
    def __init__(self,
        ...
        self.coding_len = self.angle_coder.num_classes
        self.use_sigmoid_angle = loss_angle.get('use_sigmoid', False)
        self.use_angle_negsample = use_angle_negsample
        super(AngleCLSRRetinaHead, self).__init__(**kwargs, init_cfg=init_cfg)
        self.shield_reg_angle = shield_reg_angle
        self.loss_angle = build_loss(loss_angle)

    def _init_layers(self):
        """Initialize layers of the head."""
        super(AngleCLSRRetinaHead, self)._init_layers()
    ...
    def loss_single(self, cls_score, bbox_pred, angle_cls, anchors, labels,
                    ...
                    angle_weights, pos_angle_weights, num_total_samples):
        ...
        angle_weights = angle_weights.reshape(-1)
        pos_angle_weights = pos_angle_weights.reshape(-1)
        angle_targets = angle_targets.reshape(-1)
        ...
        angle_target = self.angle_coder.encode(angle_target, pos_angle_weights).reshape(-1)
        student_angle = self.angle_pred(angle_cls, angle_target, pos_angle_weights)
        
        return loss_cls, loss_bbox, loss_angle, student_angle
    ...
    def loss(self,
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         ...
         angel_weight_list, pos_angle_weights_list) = cls_reg_targets
        num_total_samples = (
        ...
        losses_cls, losses_bbox, losses_angle, student_angle = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            angle_clses,
            ...
            angel_target_list,
            angel_weight_list,
            pos_angle_weights_list,
            num_total_samples=num_total_samples)
        s_a = np.cumsum(student_angle, axis=0)
        # print("angle_pred: {:.4f}".format(s_a[-1,0]/s_a[-1,1]))
        return dict(
            loss_cls=losses_cls,
    ...
    def _get_targets_single(self,
        ...
        pos_angle_weights = torch.zeros_like(bbox_targets[:, 4:5])
        # print("1111:", pos_angle_weights.shape)
        ...
        if len(pos_inds) > 0:
            ...
            pos_angle_weights[pos_inds, :] = 1.0

            angle_targets = self.angle_coder.encode(angle_targets, angle_weights).reshape(-1)
        ...
        if len(neg_inds) > 0:
            ...
            if self.use_angle_negsample:
                angle_weights[neg_inds, :] = 1.0
        ...
        if unmap_outputs:
            ...
            angle_weights = unmap(angle_weights, num_total_anchors,
                                  inside_flags)
            pos_angle_weights = unmap(pos_angle_weights, num_total_anchors, inside_flags)

        return (labels, label_weights, bbox_targets, bbox_weights, pos_inds,
                neg_inds, sampling_result, angle_targets, angle_weights, pos_angle_weights)
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, angle_cls, anchors in zip(
            ...
            angle_scores = angle_cls.sigmoid()
            ...
            if scores.shape[0] > nms_pre > 0:
                ...
                angle_scores = angle_scores[topk_inds, :]
        ...
        # print("mlvl_scores: ",mlvl_scores.size())
        if self.use_sigmoid_cls:
        ...
        if with_nms:
            det_bboxes, det_labels = multiclass_nms_rotated(
                ...
                cfg.max_per_img)
            # print("det_bboxes: ", det_bboxes.size())
            # print("det_bboxes: ", det_bboxes)
            # print("det_labels: ", det_labels.size())
    ...
    def angle_pred(self,
                   ...
                   angle_target,
                   angle_weight):
        angle_pred = torch.argmax(angle_cls, dim=1)
        angle_pos = angle_pred[angle_weight==1]
        pos_target = angle_target[angle_weight==1]
        # print("num_target: ",pos_target.numel())
        if pos_target.numel()==0:
            return np.array([0, 0])
        angle_len = self.coding_len
        delta1 = angle_pos - pos_target
        delta2 = (angle_pos - pos_target + angle_len + angle_len) % angle_len - angle_len
        abs_dtheta1 = torch.abs(delta1)
        abs_dtheta2 = torch.abs(delta2)
        da = torch.where(abs_dtheta1 < abs_dtheta2, abs_dtheta1, abs_dtheta2) # -25 -25\
        pos_inds = da < 3
        return np.array([angle_pos[pos_inds].numel(), angle_pos.numel()])